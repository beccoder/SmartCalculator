...
int s21_calculate_rpn(struct stack_node *rpn, double *ans) {
    ...
    while (rpn != NULL) {
        ...
        } else if (strchr("+-*/^", rpn->info[0]) || rpn->info[0] == '%') {
            double a, b;
...
struct stack_node *s21_get_rpn_from_tokens(char **tokens, int token_len) {
    ...
    for (int i = 0; i < token_len; i++) {
        ...
        } else if (strchr("+-*/^", **(tokens + i)) || **(tokens + i) == '%') {
            int prec = s21_get_prec(**(tokens + i));
            while (opers != NULL && opers->info[0] != '(' &&
                s21_get_prec(opers->info[0]) >= prec &&
                s21_get_ass(**(tokens + i)) == 'L') {
...
void reset_exp(expectations_t *exp, int num, int func, int oper,
    int open_bracket, int close_bracket, int bracket_opened) {
    if (num != -1) {
        exp->expect_num = num;
    }
    if (func != -1) {
        exp->expect_func = func;
    }
    if (oper != -1) {
        exp->expect_oper = oper;
    }
    if (open_bracket != -1) {
        exp->expect_open_bracket = open_bracket;
    }
    if (close_bracket != -1) {
        exp->expect_close_bracket = close_bracket;
    }
    if (bracket_opened != -1) {
        exp->bracket_opened = bracket_opened;
    }
}
...
char **s21_parse_to_tokens(char *str_expression, int *n) {
    ...
    if (ret_val == 1) {
        ...
        reset_exp(&expectations, 1, 1, 0, 1, 0, 0);
        ...
        while (*p != '\0') {
            if (expectations.expect_num && !regexec(&reg_exps[NUM_EXPRESSION], p, nmatch, pmatch, 0)) {
                ...
                p += (strlen(tmp));
                reset_exp(&expectations, 0, 0, 1, -1, expectations.bracket_opened, -1);
            ...
            if (expectations.expect_func) {
                ...
                for (int i = 1; i < COUNT_EXPRESSIONS; i++) {
                    if (!regexec(&reg_exps[i], p, nmatch, pmatch, 0)) {
                        ...
                        snprintf(*(tokens + count_tokens - 1), strlen(tmp) + 1, "%s", tmp);
                        p += (strlen(tmp));
                        reset_exp(&expectations, 0, 0, 0, 1, 0, -1);
                        checked = 1;
                        break;
                    }
                }
            ...
            if (expectations.expect_oper) {
                if (strchr("+-*/^", *p) || *p == '%') {
                    ...
                    *(tokens + count_tokens - 1) = malloc(5 * sizeof(char));
                    snprintf(*(tokens + count_tokens - 1), strlen(tmp) + 1, "%s", tmp);
                    p += (strlen(tmp));
                    reset_exp(&expectations, 1, 1, 0, 1, 0, -1);
                    continue;
                }
            }
            if (expectations.expect_open_bracket) {
                if (*p == '(') {
                    ...
                    snprintf(*(tokens + count_tokens - 1), strlen(tmp) + 1, "%s", tmp);
                    p += (strlen(tmp));
                    reset_exp(&expectations, 1, 1, 0, 1, 1, expectations.bracket_opened + 1);
                    continue;
                }
            }
            if (expectations.expect_close_bracket) {
                if (*p == ')') {
                    ...
                    *(tokens + count_tokens - 1) = malloc(5 * sizeof(char));
                    snprintf(*(tokens + count_tokens - 1), strlen(tmp) + 1, "%s", tmp);
                    p += (strlen(tmp));
                    reset_exp(&expectations, 0, 1, 1, 1, 1, expectations.bracket_opened - 1);
